...
def downloadchapter(href, dic, chapternumber):
    ...
    if not os.path.exists(dfolder):
        os.makedirs(dfolder)
    chaptersoup = BeautifulSoup(scraper.get(str(href.replace(".html", "-10-1.html?waring=1")), headers=ninemanga.loader.headers).text, 'lxml')
    ...
    for option in chapterpagesdiv.find_all("option"):
        ...
        chapterimagessoup = BeautifulSoup(scraper.get(chapterimagesurl, headers=ninemanga.loader.headers).text, 'lxml')
...
def main():
    ...
    for manga in ninemangalist:
        ...
        chapterlistsoup = BeautifulSoup(scraper.get(
            manga_url, headers=ninemanga.loader.headers).text, 'lxml')
