from utils.sortissues import loader
import requests
import os
import patoolib
...
from utils.sortissues.functions import scankomgalibrary
...
def treeelement(datavolume, dataissue, web):
    ...
    if dataissue["results"]["name"]:
        ET.SubElement(root, "Title").text = f'{str(dataissue["results"]["volume"]["name"]).replace("Doctor Who: The Twelfth Doctor - ", "")}'
        # ET.SubElement(root, "Title").text = dataissue["results"]["name"]
    if dataissue["results"]["volume"]["name"]:
        ET.SubElement(root, "Series").text = datavolume["results"]["name"]
    if dataissue["results"]["issue_number"]:
        # ET.SubElement(root, "Number").text = "{:0>4}".format(
        #     dataissue["results"]["issue_number"]
        # )
        ET.SubElement(root, "Number").text = "{:0>4}".format(1)
    print(datavolume)
    if datavolume["results"]["start_year"]:
        ET.SubElement(root, "Volume").text = datavolume["results"]["start_year"]
    if dataissue["results"]["description"]:
        ET.SubElement(root, "Summary").text = dataissue["results"]["description"]
...
def generatexml(datavolumes, issue):  # sourcery skip: remove-redundant-if
    headers = {
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/87.0.4280.141 Safari/537.36"
    }
    url = f"https://comicvine.gamespot.com/api/issue/{issue}/?api_key=dcb22bb374f04e7217eaca81f2fcfffbe5062e42&format=json"
    responseissue = requests.get(url, headers=headers)
    dataissue = responseissue.json()
    web = dataissue["results"]["site_detail_url"]
    if dataissue:
        treeelement(datavolumes, dataissue, web)
...
def generatecbz(my_dict):
    ...
    if not os.path.exists(f"{destino}/poster.jpg") and my_dict["ComicInfo"][
        "Publisher"
    ] in ["Marvel", "Delcourt", "DC Comics", "Titan Comics"]:
    ...
    loader.mensaj.append(f"{destino}\n\n")
...
def main():
    ...
    datavolumes = datafromcomicvine(comiclocaldb["slug"])
    ...
    patoolib.extract_archive(comiclocaldb["source"], outdir=loader.temporal)
    generatexml(datavolumes, comiclocaldb["issue-slug"])
    my_dict = loadxml()
    generatecbz(my_dict)
    try:
        shutil.rmtree(loader.temporal)
