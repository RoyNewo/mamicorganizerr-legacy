    ...
    if dic["Series"] in history:
        if issue in history[dic["Series"]]:
            ...
            if issue in komgabookid[dic["Series"]]:
                ...
                getcomicsinfo.loader.mensaj2.append(f"{dic['Series']} - {dic['provider']} - {issue}: Aun no esta en komgabookid y no se puede actualizar se hara en la siguiente ejecucion \n\n")
                return False
        else:
