...
def generatexml(dic, issuenumber):  # sourcery skip: remove-redundant-if
    ...
    for issue in datavolume["results"]["issues"]:
        if issue["issue_number"] == issuenumber.lstrip("0"):
...
def main():
    manually = ["/home/data/Comics/Tachiyomi/ReadComicOnline (EN)/Harley Quinn The Animated Series The Eat. Bang! Kill. Tour (2022)"]
    if manually:
        for manual in manually:
            ...
            for comic in files2:
                ...
                patoolib.extract_archive(
                    comic, outdir=loader.temporal)
                ic(f'Extraido: {comic}')
                issuenumber = comic.split("#")[1].split(".")[0]
                generatexml(loader.mangas[manual], issuenumber)
                my_dict = loadxml()
                if historial.historial(
                        loader.history, "{:0>4}".format(issuenumber), loader.mangas[manual], loader.komgabooksid) == True:
                    generatecbz(my_dict, comic.split("/")[-1])
                loader.save()
                try:
                    shutil.rmtree(loader.temporal)
                except Exception:
